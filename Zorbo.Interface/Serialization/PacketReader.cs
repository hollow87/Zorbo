...
namespace Zorbo.Serialization
{
    public class PacketReader : BinaryReader
    {
        ...
        public virtual T ReadObject<T>(IFormatter formatter) {

            object ret = ReadObject(typeof(T), formatter);
            return (ret != null) ? (T)ret : default(T);
        }

        private object ReadObject(Type type, IFormatter formatter) {

            var members = TypeDictionary.GetRecord(type);

            object obj = Activator.CreateInstance(type, true);
            members.ForEach(prop => prop.Property.SetValue(obj, ReadProperty(obj, prop, formatter), null));

            return obj;
        }

        private IPacket ReadPacket(IFormatter formatter) {
            try {
