...
namespace Zorbo.Serialization
{
    public class PacketWriter : BinaryWriter
    {
        ...
        public void WriteObject(object value, IFormatter formatter) {
            WriteObject(value, value != null ? value.GetType() : null, formatter);
        }

        private void WriteObject(object value, Type valType, IFormatter formatter) {
            if (valType == null)
                throw new ArgumentNullException("Type information cannot be null", "valType");

            var members = TypeDictionary.GetRecord(valType);
            members.ForEach(prop => WriteProperty(value, prop, formatter));
        }

        private void WritePacket(IPacket packet, IFormatter formatter) {
        ...
        private void WriteProperty(object value, PropertyData prop, IFormatter formatter) {
            ...
            switch (Type.GetTypeCode(prop.Property.PropertyType)) {
                ...
                case TypeCode.Object:
                    ...
                    if (ptype == typeof(byte[]))
                        ...
                        WritePacket((IPacket)propvalue, formatter);
